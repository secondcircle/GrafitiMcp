...
from typing import Any, Optional, TypedDict, Union, cast, Dict, Type, Callable, Awaitable
...
class Task(BaseModel):
    """Represents a specific task or unit of work within a project, potentially part of a sequence."""
    task_id: str = Field(..., description="A unique identifier for the task (e.g., TASK-LOGIN, ISSUE-123).")
    name: str = Field(..., description="A descriptive name or title for the task.")
    status: str = Field(..., description="The current status (e.g., TODO, IN_PROGRESS, DONE, BLOCKED).")
    project: str = Field(..., description="The project identifier this task belongs to.")
    sequence_number: Optional[int] = Field(None, description="Optional order of execution within a plan or sequence.") # ADD THIS FIELD
    description: Optional[str] = Field(None, description="Optional further details about the task.")
    assignee: Optional[str] = Field(None, description="Who is assigned to the task (e.g., Agent, User).")
